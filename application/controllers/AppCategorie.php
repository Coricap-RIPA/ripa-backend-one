...
class AppCategorie extends CI_Controller {
	...
	public function siteindex($id_categorie=null,$id_tel_user=null)
	{
		...
		if ($id_categorie == null) {
			...
			$twelves_last_articles 	= $this->getTewlveLastArticles($id_categorie);
			$total_article_number 	= $this->get_total_article_number($id_categorie);
			...
			if( !empty($categorie) AND !empty($twelves_last_articles) ){

				$class_categorie = str_replace(' ','',strtolower($categorie['nom_categorie']));

				$this->load->view("site_app_views/categorie", 
				[
	...
	public function filter($id_section=null,$id_tel_user=null)
	{
	}
	...
	public function loadMoreArticles(){
		...
		$id_categorie    		= (int) htmlspecialchars($_GET['id_categorie']);
		$cadeaux  				= $this->Article_model->get_all_articles_by_limit_offset_id_foreign_categorie(12,$index_to_load_article,$id_categorie);
