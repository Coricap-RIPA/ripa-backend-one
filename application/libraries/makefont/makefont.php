...
function MessageUser($txt, $severity='')
{
	if(PHP_SAPI=='cli')
	{
		if($severity)
...
function Notice($txt)
{
	MessageUser($txt, 'Notice');
}

function Warning($txt)
{
	MessageUser($txt, 'Warning');
}

function Error($txt)
{
	MessageUser($txt, 'Error');
	exit;
}
...
function MakeFont($fontfile, $enc='cp1252', $embed=true, $subset=true)
{
	...
	if($embed)
	{
		if(function_exists('gzcompress'))
		{
			...
			$info['File'] = $file;
			MessageUser('Font file compressed: '.$file);
		}
		else
		{
	...
	MessageUser('Font definition file generated: '.$basename.'.php');
}
