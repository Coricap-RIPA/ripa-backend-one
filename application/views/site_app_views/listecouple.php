	...
	<div class="page-content">
		...
		<div class="dz-bnr-inr" style="background-image:url(<?php echo base_url('assets/site_app_assets/images/'); ?>background/bg-shape.jpg);">
			<div class="container">
				<div class="dz-bnr-inr-entry">
					<h1>Couple / <?php echo $couple['nom_couple'];?> </h1>
					<nav aria-label="breadcrumb" class="breadcrumb-row">
						<ul class="breadcrumb">
							<li class="breadcrumb-item"><a href="<?php echo site_url('AppIndex/index/');?>"> Acceuille </a></li>
							<li class="breadcrumb-item">Couple / <?php echo $couple['nom_couple'];?> </li>
						</ul>
					</nav>
				</div>
			</div>
		...
		<section class="content-inner shop-account">
			...
			<div class="container">
				<div class="row">
					<div class="col-lg-8">
						<form action="<?php echo site_url('AppArticle/updatePanier/'); ?>" class="form_update_panier_main" name="form_update_panier_main">
							<div class="table-responsive">
								<table class="table check-tbl">
									...
									<tbody>
										...
										<?php 
											if (!empty($articles_couples)) {
												$sous_total 	= 0;
												$compter_panier = 0;
												foreach ( $articles_couples as $key => $commande) {
													$compter_panier++;
													$sous_total  		= $sous_total + ($commande['prix_vente'] * $commande['quantite']);
													$sous_total_panier  = $sous_total_panier + $sous_total;
													echo '	<tr>
																<td class="product-item-img"><a href="'.site_url('AppArticle/index/'.$commande['id_article'].'/').'"><img src="'.base_url('assets/uploads/files/'.$commande['photo_article']).'" alt="/"></a></td>
																<td class="product-item-name"><a href="'.site_url('AppArticle/index/'.$commande['id_article'].'/').'">'.$commande['nom_article'].'</a></td>
																<td class="product-item-price">'.number_format($commande['prix_vente'], 0, ',',' ').'$</td>
																<td class="product-item-quantity">
																	<div class="quantity btn-quantity style-1 me-3">
																		<input type="number" value="'.$commande['quantite'].'" name="quantite_article_'.$compter_panier.'" disabled>
																		<input type="number" value="'.$commande['id_article'].'" name="id_article_'.$compter_panier.'" style="display:none;">
																	</div>
																</td>
							...
							<div class="row shop-form m-t30">
								...
								<div class="col-md-12">
									<div class="form-group">
										<div class="input-group mb-0">
											<a href="https://wa.me/243810274370?text=Bonjour cadeauMart" class="btn btn-secondary btn-icon" style="background-color:#0d775e !important; border-color:#0d775e !important; ">
												<i class="fab fa-whatsapp"></i>
												<span class="d-md-block">JE PRENDS UN CADEAU POUR LE COUPLE </span>
											</a> 
										</div>
									</div>
								</div>
								...
								<div class="col-md-6 text-end" style="display:none;">
									...
									<input type="text"   value="<?php if($userAppData) echo $userAppData['usertel']; ?>" name="usertel" hidden>
									<input type="text"   value="<?php echo $compter_panier; ?>" name="total_article_panier" hidden>
									<button type="submit" class="btn btn-secondary btn-block m-b20">METTRE À JOUR LE PANIER</button>	
								</div>
					...
					<div class="col-lg-4">
						<h4 class="title mb15"> Liste des cadeaux </h4>
						<div class="cart-detail">
							...
							<table>
								<tbody>
									<tr class="total">
										<td>
											<h6 class="mb-0">CODE COUPLE : </h6>
										</td>
										<td class="price" id="code_couple">
											<h6 class="mb-0" id="voir_code_couple"> <?php echo $couple['code_couple'];  ?> </h6>
										</td>
									</tr>
								</tbody>
							</table>

							<a href="https://wa.me/" class="btn btn-secondary btn-icon" style="display:none;">
								<i class="fab fa-whatsapp"></i>
								<span class="d-md-block">JE COMMUNIQUE MON CODE </span>
							</a>
...
<script>
	$(function(){
		...
		function sendFormApp() {
			...
			$("form").each(function (index, element) {
				$(element).submit(function (e) { 
					...
					if (classForm == 'form_update_panier' || classForm == 'form_update_panier_main') {
						...
						$.ajax({
							...
							success: function (response) {
								if (/__OK/.test(response)) {
									...
									$("#id_sous_total_panier_main").text(tab_data_panier[2].toLocaleString()+' $');
					...
					$.ajax({
						...
						success: function (response) {
							if (/__OK/.test(response)) {
								...
								$("#id_sous_total_panier_main").text(tab_data_panier[3].toLocaleString()+' $');
		...
		function refreshPanier(data_panier) {
			...
			$("#id_sous_total_panier_main").text(tab_data_panier[4].toLocaleString()+' $');
